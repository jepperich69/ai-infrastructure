...
function Show-CommandHelp {
    ...
    $lines = switch ($n) {
        ...
        26 { @(
            "Update infrastructure from GitHub",
            "Pulls the latest version of AI_auto from origin/master.",
            "Your config.local.ps1 is gitignored and will never be overwritten.",
            "Also checks and fixes Claude Code hook paths in settings.json if needed.",
            "",
            "Safe to run at any time. Review CHANGELOG.md after upgrading.",
            "",
            "Example:",
            "  helpi 26"
        )}
...
function Invoke-Command-N {
    ...
    switch ($n) {
        ...
        5  { & "$PSScriptRoot\open_project.ps1" -Project $proj }
        6  {
               if ($texFile) { & "$PSScriptRoot\compile_latex.ps1" -Project $proj -TexFile $texFile }
               else           { & "$PSScriptRoot\compile_latex.ps1" -Project $proj }
           }
        7  {
               $projRoot        = Resolve-ProjectRoot $proj
               ...
               Pop-Location
               & "$PSScriptRoot\generate_handover.ps1" -Project $proj
        ...
        8  {
               ...
               if ($tag) { & "$PSScriptRoot\snapshot.ps1" -Project $proj -Tag $tag }
               else      { & "$PSScriptRoot\snapshot.ps1" -Project $proj }
           }
        9  {
               $rollN = 1
               if (!$Force) {
               ...
               & "$PSScriptRoot\rollback.ps1" -Project $proj -N $rollN
           }
        10 {
               $projRoot   = Resolve-ProjectRoot $proj
        ...
        16 { & "$PSScriptRoot\generate_docs.ps1" }
        17 { Show-ClaudeCheatsheet }
        18 { Toggle-ModelCheck }
        19 { if ($texFile) { & "$PSScriptRoot\generate_slides.ps1" -Project $proj -Preset $texFile }
             else          { & "$PSScriptRoot\generate_slides.ps1" -Project $proj } }
        20 { & "$PSScriptRoot\restore.ps1" }
        21 { & "$PSScriptRoot\setup.ps1" }
        22 { & "$PSScriptRoot\compress_log.ps1" -Project $proj }
        23 { if ($texFile) { & "$PSScriptRoot\push_to_github.ps1" -Project $proj -RepoName $texFile }
             else          { & "$PSScriptRoot\push_to_github.ps1" -Project $proj } }
        24 { if ($texFile) { & "$PSScriptRoot\generate_onepager.ps1" -Project $proj -TexFile $texFile -Agent $(if ($agent) { $agent } else { "gemini" }) }
             else          { & "$PSScriptRoot\generate_onepager.ps1" -Project $proj -Agent $(if ($agent) { $agent } else { "gemini" }) } }
        25 {
