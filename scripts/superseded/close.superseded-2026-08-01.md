...
Walk up from the current working directory until you find a folder that directly contains `_ai_log.md`. That folder is the project root. (This covers `Pub_`/`Pro_`/`PhD_` papers, `AI_auto`, and any generic project folder anywhere under `JR\` initialized via `helpi 1` or `helpi 27`.) Note:
- `PROJECT_NAME` — the folder name (e.g. `AI_auto`, `Pub_Lemons`, or any generic project folder name)
...
`pwsh -NoProfile -File "C:\Users\rich\OneDrive - Danmarks Tekniske Universitet\JR\AI_auto\scripts\generate_handover.ps1" -Project <PROJECT_NAME>`
