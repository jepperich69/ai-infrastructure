...
$sep  = "  " + ("-" * 65)
$aiRoot = Split-Path -Parent $MyInvocation.MyCommand.Path   # AI_auto/ folder

Write-Host ""
Write-Host "  AI Research Infrastructure -- First-time Setup" -ForegroundColor Cyan
Write-Host "  This wizard writes config.ps1 and wires up your shell profile." -ForegroundColor DarkGray
...
# Tool paths (update if installed in non-default locations)
`$vscode           = "$vscode"
`$claudeDir        = "`$env:USERPROFILE\.claude"
`$miktexBin        = "$miktexBin"
`$latexdiffScript  = "$latexdiffScript"
`$strawberryPerl   = "$strawberryPerl"
"@

Set-Content (Join-Path $aiRoot "config.ps1") $configContent -Encoding UTF8
Write-Host "  OK: config.ps1 written." -ForegroundColor Green
